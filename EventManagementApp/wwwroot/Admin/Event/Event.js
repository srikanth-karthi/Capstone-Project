...
var inst = mobiscroll.eventcalendar("#demo-desktop-month-view", {
  ...
  eventDelete: false,
  view: {
    calendar: { labels: "all", type: "month" },
  },
...
function clickevent(args) {
  const clickedEvent = data.find(
    (event) => event.scheduledEventId === args.event.eventid
  );
...
function showModal(event) {
  ...
  document.getElementById("modalDetails").innerHTML = `
    <div class="modal-grid">
      ...
      <p><strong>Client Name:</strong> ${event.clientName}</p>
        ...
        <p><strong>Food Preference:</strong> ${
          event.foodPreference || "Not Provided"
        }</p>
      ...
      <p><strong>Client Email:</strong> ${event.clientEmail}</p>
        ...
        <p><strong>Catering Instructions:</strong> ${
          event.cateringInstructions || "Not Provided"
        }</p>
        <p><strong>Special Instructions:</strong> ${
          event.specialInstructions || "Not Provided"
        }</p>
  ...
  <div class="modal-buttons">
    ${
      !event.isCompleted
        ? `<button class="completed" onclick="markAsCompleted(${event.scheduledEventId})">Mark as Completed</button>`
        : '<button class="completed" >Event Completed</button>'
    }
  </div>
    `;
...
async function markAsCompleted(eventid) {
  ...
  data.forEach((a) => {
    if (a.scheduledEventId === eventid) a.isCompleted = true;
  });
  Rendercalander();
  closeModal();
